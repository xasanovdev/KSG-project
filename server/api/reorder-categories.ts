...
export default defineEventHandler(async (event) => {
    ...
    if (!draggedItem) {
        throw createError({ statusCode: 400, message: 'No dragged item provided' });
    }
    ...
    } else if (type === 'subcategory' && draggedItem.type === 'subcategory' && draggedItem.subId !== undefined && targetSubId !== undefined) {
        const category = categories.find(cat => cat.id === draggedItem.categoryId);
        if (category && category.sub_categories) {
            const fromIndex = category.sub_categories.findIndex(sub => sub.id === draggedItem.subId);
            ...
            if (fromIndex !== -1 && toIndex !== -1) {
                ...
                category.sub_categories.forEach((sub, index) => (sub.order = index + 1));
            }
        }
    }
