...
export default defineEventHandler(async (event) => {
    ...
    if (type === 'category' && draggedItem.type === 'category') {
        ...
        const fromIndex = subCategories.findIndex(
            (sub: SubCategory) => sub.id === draggedItem.subId
        );
        const toIndex = subCategories.findIndex(
            (sub: SubCategory) => sub.id === targetSubId
        );
        ...
        if (fromIndex !== -1 && toIndex !== -1) {
            ...
            subCategories.forEach(
                (sub: SubCategory, index: number) => (sub.order = index + 1)
            );
